...
\begin{itemize}
    \item \textbf{Dataset Constraints:} The data for the training of the model were limited to Twitter posts only. There is no evidence that the model can handle other platforms (Facebook, WhatsApp), different languages, or various multimedia formats (video, audio).
    ...
    \item \textbf{Adversarial Robustness:} The model is not checked for adversarial perturbations (e.g., adding imperceptible noise to images or paraphrasing text). The next step should be to assess robustness by adversarial attacks (FGSM, PGD).
\end{itemize}
